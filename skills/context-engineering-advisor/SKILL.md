...
- **Quantitative Degradation:** Accuracy degrades significantly on long-context tasks; the exact threshold is model- and task-dependent
...
**Key Finding:** Selective retrieval (using only relevant context rather than full documents) consistently improves accuracy while reducing latency and cost — the optimal token budget varies by model and task.
...
**LLM-Powered ETL:** Models identify memory signals and propose consolidations; a validation gate must review before any auto-generated content becomes durable state, to prevent hallucinated or injected text from persisting.
...
This skill uses the following interaction protocol:
...
### Related Skills
- **[context-engineering](../context-engineering/SKILL.md)** — Core context engineering patterns and techniques
- **[discovery-process](../discovery-process/SKILL.md)** — Upstream discovery work that feeds context design
