...
Run `br config list` and `br dep cycles` to diagnose most issues.
...
# Verify all beads have descriptions (exits non-zero if any are missing)
br list --json | jq -e 'map(select(.description == "")) | length == 0'
```
