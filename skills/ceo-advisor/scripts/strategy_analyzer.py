...
class StrategyAnalyzer:
    ...
    def _create_strategic_roadmap(self, options: List, timeline_months: int) -> Dict:
        ...
        # Clamp to a minimum of 4 months; shorter timelines produce reversed
        # phase ranges and non-monotonic milestones.
        timeline_months = max(timeline_months, 4)

        # Scale phase boundaries proportionally to the provided timeline
        m1 = max(1, round(timeline_months * 3 / 18))
        m2 = max(m1 + 1, round(timeline_months / 2))
        ...
        phases = [
            ...
            {
                'phase': 'Scale',
                'months': f'{m2}-{timeline_months}',
                'focus': 'Expand and optimize',
                'initiatives': []
            },
            {
                'phase': 'Transform',
                'months': f'{timeline_months}+',
        ...
        # Define key milestones scaled to the provided timeline
        ms1 = m1
        ms2 = max(ms1 + 1, round(timeline_months / 3))
        ms3 = max(ms2 + 1, round(timeline_months * 2 / 3))
        roadmap['milestones'] = [
            {'month': ms1, 'milestone': 'Complete foundation phase', 'success_criteria': 'Core team hired, processes defined'},
            {'month': ms2, 'milestone': 'First major initiative launch', 'success_criteria': 'KPIs showing positive trend'},
            {'month': ms3, 'milestone': 'Strategic review', 'success_criteria': 'ROI demonstrated, strategy validated'},
            {'month': timeline_months, 'milestone': 'Scale achievement', 'success_criteria': 'Market position improved, financial targets met'}
        ]

        # Resource requirements
        roadmap['resource_requirements'] = {
            'leadership': 'C-suite alignment and commitment',
            'financial': f'$X million investment over {timeline_months} months',
