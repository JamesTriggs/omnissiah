...
When running this workflow as a guided conversation, use these interaction conventions:
- one-question turns with plain-language prompts
- progress labels (for example, Phase 2/8 and Activity 1/3)
- interruption handling: if the user pauses, ask "Ready to continue with Phase X?" before proceeding
...
- **Input:** Discovery insights from `skills/discovery-process/SKILL.md`
- **Participants:** PM
- **Duration:** 30 minutes
...
- **Customer journey map:** If the problem spans multiple touchpoints, map the end-to-end journey
- **Jobs-to-be-done:** If user motivations are key, document the functional, emotional, and social jobs
...
- **When:** For complex features with multiple release slices, create a user story map (backbone → walking skeleton → slices)
...
- **Participants:** PM
- **Duration:** 30 minutes
- **Output:** Epic hypothesis statement — "We believe [solution] will [outcome] because [reason]. We'll measure success by [metric]."
...
- **Duration:** 90 minutes
- **Technique:** Use Richard Lawrence's 9 splitting patterns (workflow steps, CRUD, business rules, happy/unhappy paths, cross-cutting concerns)
- **Output:** User stories split by pattern (workflow, CRUD, business rules, etc.)
...
- `skills/discovery-process/SKILL.md` — upstream input for Phase 2 problem statement
- `skills/spec-driven-development/SKILL.md` — downstream from PRD; converts outcomes into a typed implementation spec
...
**Skill type:** Workflow
**Location:** `skills/prd-development/SKILL.md`
**Dependencies:** Feeds into `spec-driven-development`; draws on `discovery-process` for problem evidence
