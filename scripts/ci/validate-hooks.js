...
const VALID_EVENTS = ['PreToolUse', 'PostToolUse', 'PreCompact', 'PostCompact', 'SessionStart', 'SessionEnd', 'Stop', 'Notification', 'SubagentStop'];
...
function validateHooks() {
  ...
  // Verify that every node script referenced in a command actually exists on disk.
  // Supports both scripts/hooks/ and skills/*/hooks/ paths.
  const PLUGIN_ROOT = path.join(__dirname, '../..');
  const pluginRootPlaceholder = /\$\{CLAUDE_PLUGIN_ROOT\}\/([^\s"]+\.js)/;
  const relativeRef = /node (scripts\/hooks\/[^\s"]+\.js)/;

  for (const [eventType, matchers] of Object.entries(typeof hooks === 'object' && !Array.isArray(hooks) ? hooks : {})) {
    if (!Array.isArray(matchers)) continue;
    for (const matcher of matchers) {
      if (!matcher.hooks) continue;
      for (const hook of matcher.hooks) {
        if (typeof hook.command !== 'string') continue;
        const m = hook.command.match(pluginRootPlaceholder) || hook.command.match(relativeRef);
        if (m) {
          const scriptFile = path.join(PLUGIN_ROOT, m[1]);
          if (!fs.existsSync(scriptFile)) {
            console.error(`ERROR: Hook command references missing file: ${m[1]} (in ${eventType})`);
            hasErrors = true;
          }
        }
      }
